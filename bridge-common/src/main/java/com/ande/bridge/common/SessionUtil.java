...
public class SessionUtil<T> {
	...
	@SuppressWarnings("unchecked")
	public  T getCompanyUser(HttpServletRequest request){
		return  (T) request.getSession().getAttribute("companyUser");
	}
	public  void setCompanyUser(HttpServletRequest request,T user){
		request.getSession().setAttribute("companyUser",user);
	}
}
