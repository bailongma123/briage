...
/**
 * Created by 丁梦洋 on 2017/9/8 0008.
 * 作用：
 */
public class DateUtil {
	...
	/**<pre>
	 * <b> 获取当前时间之前或者之后的时间，以分钟数为间隔.</b>
	 * <b>Description:</b>
	 *		minutes > 0 之后的时间
	 *		minutes < 0 之前的时间
 	 * <b>Author: chenqian@ujifu.com </b>
	 * <b>Date: 2018/1/24 13:40 </b>
	 * @param minutes 分钟
	 *<pre>
	 */
	public static Date intervalMinutes(long minutes) {
		Date nowDate = new Date();
		return new Date(nowDate.getTime() + minutes * 60000);
	}
	...
	public static Date randomDate(Date beginDate, Date endDate) {
		...
		long date = DateUtil.random(beginDate.getTime(), endDate.getTime());
		return new Date(date);
	}

	public static long random(long beginDate, long endDate) {
		long rtn = beginDate + (long) (Math.random() * (endDate - beginDate));
		//如果返回的是开始时间和结束时间，则递归调用本函数查找随机值  
		if (rtn == beginDate || rtn == endDate) {
			return random(beginDate, endDate);
		}
		return rtn;
	}
	...
	public static void main(String[] args) {
		for (int i = 0; i < 100; i++) {
			System.out.println(DateUtil.randomDate(DateUtil.date(), DateUtil.intervalMinutes(60)));
		}
		System.out.println(new Date().getTime());
		System.out.println(DateUtil.intervalMinutes(60));
